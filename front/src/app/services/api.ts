...
export async function getBottomCountries(): Promise<Country[]> {
  try {
    const response = await fetch(`${BASE_URL}/reports/bottomCountries`);
    if (!response.ok) {
      throw new Error(`Erreur HTTP : ${response.status}`);
    }
    const data: [string, number][] = await response.json();
    ...
    console.error("Erreur lors de l'appel à l'API getBottomCountries :", error);
    throw error;
  }
}
/**
